...
状态：**§3 / §4 已实现（2026-07-28，分支 `feature/splat-asset-split`），尚未在 Unity 中跑过验收测试**
日期：2026-07-24
...
| 章节 | 状态 |
|---|---|
| §3 导入期切分 + prefab 生成 | ✅ 已实现，Roslyn 编译通过 |
| §4 运行时过大资产防御检查 | ✅ 已实现 |
| §5 块间绘制顺序验证 | ⬜ 待验证（不需要新代码，需在 Unity 中目视检查） |
| §6 可选优化（视锥剔除 / 错帧排序 / SOG 导入器） | ⬜ 未做，二期 |
| §7 场景迁移 | ⬜ 待手工执行 |
| §8 验收测试 T1–T8 | ⬜ 全部待跑（需要 Unity 编辑器 + Quest 设备） |

与 §3.3 的骨架相比，实现上有一处有意的偏离：**切片采用复制而非 `GetSubArray`**。
`CalcChunkDataJob` 会就地改写 splat 数据，而 `GetSubArray` 视图在 job 安全系统下是否保持可写
随 Unity 版本而异（§3.3 坑位 1 已预判）。实现改为每片 `NativeArray.Copy` 出一份独立缓冲，
用完即 `Dispose`，同一时刻只多占一片的内存。单片（不切分）路径仍直接使用输入数组，
行为与改动前逐字节一致。
